...
/****************************************************自定义的方法***********************************************************/
    ...
    public function upload(){
        ...
        $upload->savePath =  './Uploads/images/face-icon/';// 设置附件上传目录
        if(!$upload->upload()) {// 上传错误提示错误信息
            // $this->error($upload->getErrorMsg());
            return array(1,$upload->getErrorMsg());
        }else{// 上传成功 获取上传文件信息
            return array(0,$upload->getUploadFileInfo());
        }
        // $User->photo = $info[0]['savename']; // 保存上传的照片根据需要自行组装
    }
    //获取访问者的地址
    public function ipArea(){
        $str = file_get_contents("http://api.map.baidu.com/location/ip?ak=P5DNCRqHRTkqpH47PzrteNcuaj4qwyRD");
